...
Commands:
  ...
  profile-transaction         Profile the gas usage of a transaction. Unless an output filepath is not specified, outputs a file `gas_profile_{tx_digest}_{unix_timestamp}.json` which can be opened in a flamegraph tool
                                  such as speedscope
  remove-address              Remove an existing address by its alias or hexadecimal string. Warning: removes the private key from the keystore with no way to recover it if it was not backed up
...
- `pay-` commands typically deal with coins and handle [coin merging](../../iota-101/transactions/ptb/optimizing-gas-with-coin-merging.mdx) for you, whereas `transfer` commands can handle the transfer of any object that has public transfer, meaning any object that has the `store` ability.
