...
- We import our helper get_funded_client() from utils.rs, which gives us a ready-to-use client with faucet funds.
- A Federation is the top-level trust group (in this case, all universities).
- Here we create a federation named "University Consortium".
...
- Properties define the kinds of claims that can be issued in this federation.
- "degree" is a string with allowed values: bachelor, master, phd.
- "field" is a string: cs, math, biology.
- "gpa" is numeric.

These constraints make validation straightforward later.
