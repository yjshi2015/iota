...
turn, wraps the [`address` type](../../../developer/references/framework/std/address.mdx). The UIDs on IOTA are guaranteed to be unique, and can't be reused
...
When talking about `UID` we should also mention the `ID` type. It is a wrapper around the [`address`](../../../developer/references/framework/std/address.mdx)
