...
fn should_continue_sequence_after_error(context: &mut Context, diag: Diagnostic) -> bool {
    context.add_diag(diag);
    // This is intended to handle a rather specific case when a valid sequence item
    // is on the following line from the parsing error. This is particularly
    // useful for the IDE use case when a programmer starts typing an incomplete
    // (and unparsable) line right before the line containing a valid expression.
    // In this case, we would like to still report the error but try to avoid
    // dropping the valid expression itself, particularly as it might lead to
    // unnecessary cascading errors to appear if this expression is a variable
    // declaration as in the example below where we want to avoid `_tmp1` being
    // undefined in the following lines.
    //
    // let v =
    // let _tmp1 = 42;
    ...
    {
        // if the last token was preceded by EOL, and it's in the start set for sequence
        // items, continue parsing the sequence
        return true;
    }
    false
}
