...
impl ControlFlow {
    ...
    // Combinse control flows around sequence entries (assuming `self` comes before
    // `next`):
    ...
    //     ... if (cond) break 'a; break 'b; ... Produces Divergent({a, b})
    //     divergence,
...
fn tail_block(context: &mut Context, seq: &VecDeque<T::SequenceItem>) -> ControlFlow {
    ...
    if stmt_flow.is_some() {
        // If we have statement flow and a final expression, we might have an
        // unnecessary semicolon if `last` is just a unit value. Let's check for
        // that.
...
fn value(context: &mut Context, e: &T::Exp) -> ControlFlow {
    ...
    match e_ {
        ...
        E::Loop {
            ...
            // A loop can yield values, but only through `break`. We treat the body as a
            // statement, but then consider if it ever breaks out. If it does
            // not, this is an infinite loop.
    ...
    match e_ {
        ...
        E::While(name, test, body) => {
            ...
            if context.maybe_report_value_error(&mut test_flow) {
                ...
                // Since a body for a While loop is only Possible, not certain, we always force
                // it into Possible.
