...
// resolution of all named addresses in the package graph in scope. So we can
// simply grab all of the source files across all packages and build the Move
// model from that. TODO: In the future we will need a better way to do this to
// support renaming in packages where we want to support building a Move model.
