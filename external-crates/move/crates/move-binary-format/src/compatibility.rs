...
// ***************************************************************************
//
// If `check_datatype_layout` is false, type safety over a series of upgrades
// cannot be guaranteed for either structs or enums.
// This is because the type could first be removed, and then re-introduced with
// a diferent layout and/or additional variants in a later upgrade. E.g.,
...
/// `{ check_datatype_layout: true, check_private_entry_linking: true }`: fully
/// backward compatible `{ check_datatype_layout: true,
/// check_private_entry_linking: false }`: Backwards compatible, private entry
/// function signatures can change `{ check_datatype_layout: true,
/// check_private_entry_linking: true }`: Backward compatible, exclude the
/// friend module declare and friend functions `{ check_datatype_layout: true,
/// check_private_entry_linking: false }`: Backward compatible, exclude the
/// friend module declarations, friend functions, and private and friend entry
/// function
...
impl Compatibility {
    ...
    pub fn framework_upgrade_check() -> Self {
        Self {
            check_datatype_layout: true,
            // Checking `entry` linkage is required because system packages are updated in-place,
            // and a transaction that was rolled back to make way for reconfiguration
            // should still be runnable after a reconfiguration that upgraded the
            // framework.
            //
            // A transaction that calls a system function that was previously `entry` and is now
            // private will fail because its entrypoint became no longer callable. A
            // transaction that calls a system function that was previously `public
            // entry` and is now just `public` could also fail if one of its mutable
            // inputs was being used in another private `entry` function.
            check_private_entry_linking: true,
...
impl InclusionCheck {
    ...
    pub fn check_with_mode<M: InclusionCheckMode>(
        ...
        // Since the structs are sorted we can iterate through each list to find the
        // differences using two pointers
        ...
        // friend checks, keeping in line with the previous implementation only checking
        // for length differences. will need followup work and a protocol
        // version for more detailed friend checks.
    ...
    std::iter::from_fn(move || match (old.peek(), new.peek()) {
        (Some((old_key, _old_value)), Some((new_key, _new_value))) => match old_key.cmp(new_key) {
            std::cmp::Ordering::Equal => {
                // Unwrap is safe because we know there is a next element since we just peeked
                // it.
