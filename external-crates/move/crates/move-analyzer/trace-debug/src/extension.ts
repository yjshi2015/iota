...
async function findTraceInfo(editor: vscode.TextEditor): Promise<string> {
    ...
    if (openedFileExt === JSON_FILE_EXT) {
        // this is hopefully a trace in JSON format (we do check if JSON schema
        // is correct in the following code)
...
function constructTraceInfo(tracePath: string, tracedFunctionInfo: TracedFunctionInfo): string {
    ...
    if (tracedFileBaseName === REPLAY_TRACE_FILE_NAME) {
        // replay tool generates a trace file named 'REPL_TRACE_FILE_NAME'.json
        ...
        // trace files generated when running unit tests have the following format:
        // `<package>__<module>__<function_name>.json`
